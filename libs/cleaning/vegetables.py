def clean_vegetables():
    ...
    filepath = Path(paths["vegetables"]["raw"])
    ...
    print(f"Number of distinct units: {units_qt}. The expected value is 1.")
    print(f"Number of fruits missings: {no_names}. Expected value is 0\n")


    try:
        if(sum_errors != 0 or units_qt != 1):
            raise ValueError("some of the data is wrong. Please check if the cleaning is working correctly.\n")
        df.to_csv("./data/clean/vegetable_prices.csv", header=True, chunksize=CHUNK_SIZE, index = False, encoding=encode)
        print("INFO - Cleaning done succefully, please check it at ./data/clean/\n")
